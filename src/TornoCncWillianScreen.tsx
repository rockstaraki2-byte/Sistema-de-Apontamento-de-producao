...
export function TornoCncWillianScreen({
  db,
  currentUser,
}: {
  ...
  const [selectedOperator, setSelectedOperator] = useState<string>("Willian");
  ...
  const [startOperator, setStartOperator] = useState<string>("Willian");
  ...
  const myPcpPlans = db.coilCuttingPlans?.filter(p => p.type === "TORNO_CNC_WILLIAN" && p.status === "PENDENTE") || [];
  const pendingPlans = db.coilCuttingPlans?.filter(p => p.status !== "CONCLUIDO" && p.type === "TORNO_CNC_WILLIAN") || [];
  ...
  const finalizeLog = (
    ...
    db.addLogs([
      {
        ...
        parentItemId: isManual && mParent ? Number(mParent) : undefined,
        orderId: activePack && activePack.orderId ? Number(activePack.orderId) : undefined,
    ...
    if (isManual && mParent) {
      db.addNotification({
        message: `Torno CNC: ${qty} un. de ${mTitle} p/ produto ${db.items.find((i) => i.id === mParent)?.name} (${mProcess}) via ${finalOperatorName}`,
        read: false,
      });
  ...
  const handleFinish = (updateOS: boolean, startNext: boolean, isPartial: boolean = false) => {
    ...
    const isManual = (activePack as any).isManual || activePack.itemId === 0;
    ...
    finalizeLog(
      ...
      isManual,
      {
        manualTitle: isManual ? (activePack.customProductName || activePack.partName) : undefined,
        parentItemId: isManual ? (activePack.itemId > 0 ? activePack.itemId : undefined) : undefined,
    ...
    if (isPartial) {
      ...
      db.addNotification({
        message: `Apontamento Parcial no Torno CNC Willian: ${packQuantity} peças computadas. O lote segue ativo.`,
        read: false,
      });
  ...
  const handleManualProduction = () => {
    ...
    db.addActivePack({
      ...
      orderId: selectedManualOrderId || undefined,
      color: "N/A",
      size: "N/A",
      variation: "N/A",
      operatorId: currentUser.id,
    ...
    db.addNotification({
      message: `Lote Avulso de Torno CNC Willian iniciado (${targetName})`,
      read: false,
    });
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border w-full flex flex-col gap-4 text-center">
          ...
          <div>
            <h3 className="font-bold text-xl text-gray-800">
              {activePack.itemId === 0 || (activePack as any).isManual ? "Lançamento Avulso de Componente" : activePack.partName || "OS Desconhecida"}
            </h3>
            {(activePack.itemId === 0 || (activePack as any).isManual) && (
              <div className="mt-2 text-left bg-slate-50 p-3 rounded-lg border border-slate-100 flex flex-col gap-1 inline-block text-xs mx-auto">
                <div>Componente: <strong className="text-indigo-900">{activePack.customProductName || activePack.partName}</strong></div>
                {activePack.thirdPartyName && (
                  <div>Identificação / Vínculo: <strong className="text-indigo-900">
                    {activePack.thirdPartyName}
                  </strong></div>
                )}
              </div>
  ...
  if (view === "NEW_PACK") {
    ...
    const filteredPlans = myPcpPlans.filter(p => {
      const searchStr = normalizeString(p.name);
      return searchStr.includes(normalizeString(searchTerm));
    });
    ...
    return (
      <div className="flex flex-col h-full p-2 max-w-lg mx-auto w-full relative overflow-y-auto">
        ...
        <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center gap-2">
          <Search size={20} /> Iniciar Produção de Componente ou Lote
        </h2>
        ...
        <div className="flex-1 overflow-y-auto w-full">
          {filteredGroups.length === 0 && filteredPlans.length === 0 ? (
            <p className="text-gray-500 text-center mt-4">
              Nenhum produto ou lote pendente.
            </p>
          ) : (
            <div className="grid gap-3">
              {filteredPlans.map(plan => (
                <div
                  key={`plan-${plan.id}`}
                  onClick={() => startPcpPlan(plan.id)}
                  className="bg-amber-50 p-4 border border-amber-200 flex flex-col justify-between items-start rounded-lg shadow-sm cursor-pointer hover:border-amber-400 hover:shadow-md transition gap-2 text-left"
                >
                  <span className="text-[10px] tracking-wide font-extrabold text-amber-900 bg-amber-100 border border-amber-200 px-2 py-0.5 rounded w-fit uppercase">
                    Lote PCP Direcionado
                  </span>
                  <span className="font-bold text-slate-800 text-lg">
                    {plan.name}
                  </span>
                  <span className="text-xs text-slate-600 font-semibold mt-1">
                    Quantidade Alvo: <strong className="text-amber-700">{plan.targetQuantity || 'N/A'}</strong>
                  </span>
                </div>
              ))}
    ...
    <ScreenLayout className="bg-slate-50 relative">
      ...
      >
        ...
        <div className="flex items-center gap-2.5 md:gap-3 bg-gradient-to-r from-violet-600 to-indigo-500 p-3 md:p-4 rounded-xl text-white shadow-md shrink-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] md:text-xs text-indigo-100 font-mono truncate">
              Operador: {currentUser.name} | Máquina Willian Ativa
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 shrink-0">
          ...
          <MachineStopWidget db={db} currentUser={currentUser} machineName="Torno CNC Willian" />
        </div>
